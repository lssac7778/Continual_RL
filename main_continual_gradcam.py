...
scores = []; losses = []; cam_losses = []; total_steps = 0; env_index = 0
...
for i in range(1, n_rounds):
    ...
    if i%env_change_interval==0 and len(envs)-1 > env_index:
        ...
        cont_frames, cont_actions, _ = collect_traj(agent, env, traj_len, device)
        ...
        storage.append(env_index, cont_frames, cont_actions, torch.from_numpy(cont_cams))
    ...
    for e in range(n_opt_epochs):
        ...
        """Train through Grad CAM loss"""
        
        if env_index > 0:
            ctrain_frames, ctrain_actions, ctrain_cams = storage.get_batch()
    ...
    if len(cam_losses) > 0:
        avg_cam_loss = sum(cam_losses[-n_opt_epochs:])/len(cam_losses[-n_opt_epochs:])
        writer.add_scalar('avg_loss/cam', avg_cam_loss, total_steps)
